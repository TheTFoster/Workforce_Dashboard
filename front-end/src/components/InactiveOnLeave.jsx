// front-end/src/components/InactiveOnLeave.jsx
...
// prebundle/interop errors that can cause runtime failures.
...
export default function InactiveOnLeave() {
  ...
  // load: try server-filtered inactive, else fallback to all
  useEffect(() => {
    let cancel = false;
    (async () => {
      try {
  ...
  // base list: inactive/on leave
  const baseList = useMemo(
    () => rows.filter((r) => statusNorm(r) === "inactive"),
    [rows]
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn("react-window not available for InactiveOnLeave; falling back.", err);
      }
    })();
    return () => {
      alive = false;
          ...
          <div className={styles.rowInner} style={{ height: ITEM_HEIGHT }}>
            <div className={styles.employeeItem}>
              <div className={styles.employeeInfo}>
                ...
                {/* Phone: link + copy ⎘ (same as Home/Terminated) */}
      ...
      <main className={styles.main}>
        <div className={styles.contentWrapper}>
          <div className={styles.subtitleRow}>
            <h2 className={styles.subtitle}>Inactive / On Leave</h2>
          ...
          {filtered.length === 0 ? (
            <p className={styles.noData}>No inactive / on-leave employees.</p>
          ) : VirtualListComp ? (
            <VirtualListComp
      ...
      <Footer
        ...
        rightSlot={
          <div className={footerStyles.linkRow}>
            ...
            <span className={footerStyles.pageTag}>
              Inactive / On Leave View
            </span>
          </div>
        }
      />
